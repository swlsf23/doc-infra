...
| Path | Purpose |
| --- | --- |
| [`convert/pipeline/`](convert/pipeline/) | Python package: manifest parsing, converters, [`convert`](convert/pipeline/convert.py) CLI logic |
| [`convert/config.yml`](convert/config.yml) | Shared paths and options for scripts |
| [`convert/generate_manifest.py`](convert/generate_manifest.py) | Scans Markdown under `input_dir` and writes [`content/manifest.yml`](content/manifest.yml) |
| [`convert/convert.py`](convert/convert.py) | Stage 1: converts each manifest page from Markdown to **raw** HTML |
| [`convert/build_site.py`](convert/build_site.py) | Stage 2: wraps raw HTML with navigation and copies [`ux/`](ux/) assets into `site_output_dir` |
...
Optional: `python convert/generate_manifest.py --config /path/to/config.yml`

## Convert Markdown to HTML

Uses the manifest and [`pipeline.converters.registry`](convert/pipeline/converters/registry.py) (Markdown backend under [`markdown/mistune/`](convert/pipeline/converters/markdown/mistune)): default **`markdown`** is [Mistune](https://github.com/lepture/mistune) (pure Python). Each page becomes a minimal HTML5 document under `html_output_dir`, mirroring paths (`guides/foo.md` → `guides/foo.html`).

```bash
python convert/convert.py
```

Optional: `python convert/convert.py --config /path/to/config.yml`

**Admonitions** (styled callouts) use a fenced block: an opening line `::: caution` (or `warning`, `note`), an optional title on the same line, body text, then a closing `:::` on its own line. Body content supports **inline** Markdown (emphasis, links, code). See [`directives.py`](convert/pipeline/converters/markdown/mistune/directives.py).
...
Optional: `python convert/build_site.py --config /path/to/config.yml`
...
- **`site_github_url`**: Header **GitHub** pill (icon + label). Use a quoted URL. If the key is omitted or set to an empty string, a **default** URL is used (`pipeline.site.DEFAULT_SITE_GITHUB_URL`). Set to YAML **`false`** to hide the GitHub control.
- **`converter`**: Converter id (default `markdown`). Register new converters in [`convert/pipeline/converters/`](convert/pipeline/converters/).
...
Manifest shape and behavior are documented in [`convert/pipeline/manifest.py`](convert/pipeline/manifest.py) and the docstring at the top of [`convert/generate_manifest.py`](convert/generate_manifest.py).
